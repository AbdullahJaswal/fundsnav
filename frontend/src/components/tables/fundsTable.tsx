...
const columns = [
  columnHelper.accessor('name', {
    ...
    cell: fund => (
      <Link
        href={`/dashboard/mutual-funds/fund/${fund.row.original.slug}`}
  ...
  columnHelper.accessor('amc.name', {
    ...
    cell: amc => (
      <Link
        href={`/dashboard/mutual-funds/amc/${amc.row.original.amc?.slug}`}
    ...
    <>
      <div className='flex flex-row gap-2'>
        ...
        <div className='min-w-max max-w-sm'>
          <label className='label'>
            <span className='label-text font-bold text-muted mr-2'>
              Page Size:
            </span>
            <select
                ...
                {headerGroup.headers.map(header => (
                  <th key={header.id} className="truncate font-bold">
                    ...
                    >
                      ...
                      {{
                        asc: <span className='ml-1 text-success flex items-center'>
                          <BsFillCaretUpFill />
                        </span>,
                        desc: <span className='ml-1 text-error flex items-center'>
                          <BsFillCaretDownFill />
                        </span>,
      ...
      <div className="flex flex-row gap-2 items-center justify-center">
        <div className="join">
          ...
          <button className="join-item btn normal-case disabled:bg-base-200 disabled:text-base-content disabled:font-bold" disabled>
        ...
        <span className="flex items-center">
          <label className="label">
            <span className="label-text text-muted font-bold">
              Go to page:
            </span>
            <input
              type="number"
